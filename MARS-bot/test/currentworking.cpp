...
void moveBackward(float dist_mm, int speed) {
  ...
  encL = encR = 0;
  resetPID();
  rlog("[BWD] %.0fmm target:%ld ticks", dist_mm, target);
  udpReply("ack");
 
  while (true) {
    // use abs() — backward encoders count negative
    long absL = abs(encL);
    long absR = abs(encR);
    ...
    // // error: difference between how far each wheel has travelled
    // // use abs counts so sign is consistent regardless of direction
    // float err = (float)(absL - absR);
    // int   cor = pidCalc(err);
 
    // clamp correction so neither motor drops below MIN_SPEED
    // cor = constrain(cor, -(spd - MIN_SPEED), (spd - MIN_SPEED));
 
    // negative speed = backward for motorLeft/motorRight
    motorLeft (-spd );
    motorRight(-spd );
...
void handleUDP() {
  ...
  if      (strcmp(cmd, "FORWARD")  == 0) moveForward (dist, BASE_SPEED);
    ...
    myServo.write(0);    // close — grab load
    rlog("[GRAB]");
    udpReply("done");
  }
  else if (strcmp(cmd, "RELEASE")  == 0) {
    myServo.write(90);   // open — release load
    rlog("[RELEASE]");
    udpReply("done");
  }
