...
ARRIVE_DIST_LOAD = 120    # mm — stop at side of load
ARRIVE_DIST_DROP = 160   # mm — anywhere inside 300x300 drop zone
...
class TaskManager(Node):
    def __init__(self):
        ...
        self.get_logger().info('TaskManager ready — Bot1, nearest load first')
    ...
    # ── Arrive distance based on phase ───────────────────────────
    ...
    def loop(self):
        ...
        # BACKING: reverse 15cm after releasing load
        if self.phase == BACKING:
            if self.busy:
                return
            self.get_logger().info('Backing up 150mm...')
        ...
        # CHECKING: wait for camera to settle after move
        ...
        if dist < self.arrive_dist():
            if self.phase == TO_LOAD:
                ...
                self.get_logger().info('=== At dropzone — RELEASING ===')
        ...
        if self.state == TURNING:
            if abs(angle_err) > ANGLE_THRESH:
                ...
                return   # wait for done before driving
            self.state = DRIVING
            return

        # DRIVING
        if self.state == DRIVING:
            arrive = self.arrive_dist()
